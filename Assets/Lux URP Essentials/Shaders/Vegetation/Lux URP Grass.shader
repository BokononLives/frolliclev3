...
// In case grass is added using the terrain engine it will not receive any light if Light Layers are enabled
// URP 12.1.8. only, woks fine in 13.x
[Space(5)]
[Toggle(_NOLIGHTLYERS)] _NoLightLayers("No Light Layers (URP 12 only)", Float) = 0
...
#pragma shader_feature_local _NOLIGHTLYERS
